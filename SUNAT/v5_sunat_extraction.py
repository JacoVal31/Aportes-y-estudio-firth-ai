...
BASE_DIR = "SUNAT"
V2_DIR = os.path.join(BASE_DIR, "v3")
...
def flush_log_buffer_to_file(force_flush=False):
    ...
    with log_lock:
        # Escribe si hay 100 o más logs, o si es la escritura final (force_flush).
...
def download_file(session, file_info):
    ...
    if os.path.exists(filepath):
        # Incluso si ya existe, lo contamos para el progreso.
        with log_lock:
...
def main():
    """Función principal que orquesta la recolección secuencial y la descarga concurrente."""
    ...
    with requests.Session() as session:
        ...
        try:
            ...
            print(f"Página 1: Encontrados {len(files_on_page)} archivos.")
            ...
            for page_num in range(2, TOTAL_PAGES + 1):
                ...
                files_to_download.extend(files_on_page)
                print(f"Página {page_num}: Encontrados {len(files_on_page)} archivos.")
                time.sleep(0.5) # Pausa cortés entre solicitudes.
        
        except requests.exceptions.RequestException as e:
            print(f"Error crítico durante la recolección de datos: {e}")
            return
        except KeyboardInterrupt:
            print("\n🛑 Proceso de recolección interrumpido.")
            return
    ...
    with requests.Session() as download_session:
         ...
         with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            # Creamos un futuro por cada archivo a descargar.
    ...
    # Escritura final de cualquier log que haya quedado en el búfer.
