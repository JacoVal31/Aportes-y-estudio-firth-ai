...
LOG_DIR = os.path.join(V7_DIR, "logs")
DOWNLOAD_ROOT_DIR = os.path.join(V7_DIR, "PDF descargados")

# Ruta al archivo Excel y al archivo de logs.
EXCEL_FILE_PATH = os.path.join(V7_DIR, "nombres_identificados.xlsx")
LOG_FILE_PATH = os.path.join(LOG_DIR, "logs.txt")

# URL del servidor para la descarga.
...
def download_file(session, file_info, total_files):
    """Descarga un único archivo de forma secuencial con el formato de número corregido."""
    global downloaded_files_count
    
    # --- LA CORRECCIÓN CLAVE ESTÁ AQUÍ ---
    # Aseguramos que el número tenga 6 dígitos con ceros a la izquierda usando zfill(6).
    numero_formateado = str(file_info['Numero']).zfill(6)
    
    params = {
        'cod_tipclas': file_info['Tipo'],
        'num_resclas': numero_formateado, # Usamos el número formateado.
    ...
    try:
        ...
        if response.status_code == 200 and response.headers.get('Content-Type', '').strip().lower() == 'application/pdf':
            ...
            error_log_entry = f"[Página {page_num}] [Error de Descarga] Archivo: {filename} | Estado: {response.status_code} | Content-Type: {content_type}"
...
def main():
    ...
    print("🚀 Estructura de carpetas 'v7' verificada.")

    if not os.path.exists(EXCEL_FILE_PATH):
        print(f"❌ ERROR: No se encontró el archivo de entrada en '{EXCEL_FILE_PATH}'.")
        print("Por favor, asegúrate de que 'nombres_identificados.xlsx' esté en la carpeta 'SUNAT/v7/'.")
        return
    ...
    with requests.Session() as session:
        print("\n🔑 Preparando la sesión con el servidor...")
        ...
        try:
            ...
            print("✅ Sesión preparada exitosamente.")
        except requests.exceptions.RequestException as e:
